...
//  MovieQuiz
//
//  Created by Magomed on 26.08.2026.
//
...
final class MovieQuizPresenter: QuestionFactoryDelegate {
    ...
    init(
        ...
        questionFactoryCreator: ((QuestionFactoryDelegate) -> QuestionFactoryProtocol)? = nil
    ) {
        self.viewController = viewController
        self.statisticService = statisticService
        
        if let questionFactoryCreator = questionFactoryCreator {
            self.questionFactoryCreator = questionFactoryCreator
            self.questionFactory = questionFactoryCreator(self)
        } else {
            self.questionFactoryCreator = { delegate in
